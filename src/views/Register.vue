...
async function register() { // Função para registar
  const userSt = userStore(); // Chama a store do usuário
  error.value = []; // Limpa os erros
  ...
  return userSt.register(email.value, username.value, password.value) // Chama a função de registo da store
    .then(() => {
      router.push('/');
    })
    .catch((err) => {
  ...
  <div class="bg-gradient-to-b from-sky-300 to-slate-400 h-screen w-screen flex justify-center items-center">
    <div class="flex flex-col lg:flex-row w-[72rem] h-[46rem] rounded-xl overflow-hidden shadow relative">
      ...
      <div class="w-full lg:w-7/12 mx-auto bg-white flex flex-col items-center">
        <h1 class="text-gray-900 text-3xl font-poppins font-bold mt-10 lg:mt-20 text-center lg:text-left">Preencha os
          seus dados</h1>
          ...
          <div class="flex items-start mb-6">
            <div class="flex items-center h-5">
              <input id="terms" type="checkbox" value=""
                class="w-4 h-4 border border-gray-300 rounded-full bg-gray-50 focus:ring-2 focus:ring-pink-600 text-pink-600"
                required />
            </div>
            <label for="terms" class="ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">Eu aceito os Termos e
              Condições.</label>
          </div>
          <div
            class="flex flex-col items-center space-y-4 mt-auto mb-32 lg:flex-row lg:items-center lg:space-x-4 lg:space-y-0">
            <input type="submit" value="Criar Conta"
              class="text-white text-2xl font-semibold font-poppins bg-gray-900 rounded-full w-56 h-16">
            <RouterLink to="/login">
              <button
                class="text-white text-2xl font-semibold font-poppins bg-gray-900 rounded-full w-56 h-16 lg:hidden">
                Entrar
              </button>
            </RouterLink>
          </div>
      ...
      <div v-if="error.length > 0"
        class="absolute top-0 left-0 w-full h-full flex justify-center items-center bg-black bg-opacity-50 z-50">
        <div class="bg-white p-6 rounded-lg shadow-md">
          <p class="text-gray-900" v-for="(text, i) in error" :key="i">{{ text }}</p>
          <button @click="error = []"
            class="mt-4 px-4 py-2 bg-gray-900 text-white rounded-md mx-auto block">Fechar</button>
        </div>
      </div>
      <!--  Login  -->
      <div class="hidden lg:flex w-5/12 bg-gray-900 text-white flex-col items-center">
        ...
        <div class="relative flex flex-col items-center mt-auto mb-32">
          <RouterLink to="/login">
            <button
              class="text-white text-2xl font-semibold font-poppins bg-gray-900 rounded-full w-48 h-16 border-white border-2 mt-auto">
              Entrar
            </button>
          </RouterLink>
