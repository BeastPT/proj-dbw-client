...
async function login() { // Função para fazer login
  try {
    const userSt = userStore(); // Chama a store do usuário
    await userSt.login(username.value, password.value); // Chama a função de login da store
    router.push('/'); // Redireciona para a página inicial
  } catch (err) {
    console.error(err);
    error.value = err; // Mostra o erro se der erro
  }
}
</script>
    ...
    <div class="flex flex-col lg:flex-row w-[72rem] h-[46rem] rounded-xl overflow-hidden shadow relative">
      ...
      <div class="hidden lg:flex w-5/12 bg-gray-900 text-white flex-col items-center">
        ...
        <div class="text-center mx-14 mt-16 text-base font-normal">
          <p>Faça login na sua conta com o seu nome de utilizador e senha para explorar todas as funcionalidades
            disponiveis </p>
        </div>
        ...
        <div class="flex flex-col items-center mt-16">
          <RouterLink to="/register">
            <button
              class="text-white text-2xl font-semibold font-poppins bg-gray-900 rounded-full w-48 h-16 border-white border-2 mt-8">
              Criar Conta
            </button>
          </RouterLink>
        </div>
      ...
      <div class="w-full lg:w-[65%] bg-white flex flex-col items-center">
        <h1 class="text-gray-900 text-3xl font-poppins font-bold mt-20 sm:text-5xl text-center mx-auto">Entre com a sua
          conta</h1>
        <form @submit.prevent="login" class="flex flex-col items-center h-full mt-36">
          ...
          <Field class="w-full lg:w-[120%]" type="password" placeholder="Senha" autocomplete="current-password"
            v-model="password">
            <template #icon>
              <IPassword />
            </template>
          ...
          <input type="submit" value="Entrar"
            class="text-white text-2xl font-semibold font-poppins bg-gray-900 rounded-full w-56 h-16 mb-4">

          <div class="flex flex-col items-center mt-auto mb-40 lg:hidden">
            <RouterLink to="/register">
              <button
                class="text-white text-2xl font-semibold font-poppins bg-gray-900 rounded-full w-56 h-16  mt-auto">
                Criar Conta
              </button>
            </RouterLink>
          </div>
