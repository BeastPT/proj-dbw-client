...
<template>
  <div class="bg-gray-900 w-full flex flex-col text-white">
        ...
        <nav class="mt-4 lg:mt-6 text-zinc-400 text-xl font-semibold">
          <ul :class="{ 'justify-center': show[index], 'justify-start': !show[index] }">
            <router-link v-for="(category, categoryIndex) in column.categories" :key="categoryIndex" :to="category.path"
              :class="{ 'hidden lg:flex': !show[index], 'block lg:inline-block lg:flex show-small': show[index] }">
              <li>{{ category.name }}</li>
            </router-link>
          </ul>
        </nav>
    ...
    <div class="flex flex-col lg:flex-row items-center mx-4 lg:mx-48 mt-8 lg:mt-32 border-t-4 border-t-white p-6">
      <span class="text-xl font-bold text-center lg:text-left lg:w-1/3"><img src="/images/skillswap.png"
          alt="SkillSwapp Logo" class="h-14"></span>
      <span class="text-xl font-bold mt-4 lg:mt-0 text-center lg:text-center lg:w-1/3">Madeira, Portugal</span>
      <router-link to="/privacy" class="text-sm font-light mt-4 lg:mt-0 text-center lg:text-right lg:w-1/3">Política de
        Privacidade e Cookies</router-link>
